...
class book():
    def __init__(self, root):
        ...
        self.name = tk.Entry(addFrame, bd=2, width=18, font=("Arial",15))
        ...
        self.edition = tk.Entry(addFrame, bd=2, width=18, font=("Arial",15))
        self.edition.grid(row=1, column=1, padx=10, pady=30)

        priceLbl = tk.Label(addFrame, text="Price:", bg=self.clr(100,140,240),font=("Arial",15,"bold"))
        priceLbl.grid(row=2, column=0, padx=20, pady=30)
        self.price = tk.Entry(addFrame, bd=2, width=18, font=("Arial",15))
        ...
        self.quant = tk.Entry(addFrame, bd=2, width=18, font=("Arial",15))
        self.quant.grid(row=3, column=1, padx=10, pady=30)
        ...
        nameLbl2 =tk.Label(self.detFrame, text="Book_Name:", bg=self.clr(100,240,140),font=("Arial",15,"bold"))
        ...
        optLbl.grid(row=0, column=2, padx=10, pady=20)
        self.options =ttk.Combobox(self.detFrame, width=14, font=("Arial",15),values=("First","Second","Third"))
        self.options.set("Select Edition")
        self.options.grid(row=0, column=3, padx=5, pady=20)
    ...
    def insertFun(self):
        ...
        if name and edi and pri and qu:
            ...
            try:
                self.dbFun()
                self.cur.execute("insert into books(name,edition,price,quant) values(%s,%s,%s,%s)",(name,edi,price,quant))
                self.con.commit()
                tk.messagebox.showinfo("Sucess", f"Book.{name} is Added Successfuly in Store!")
    ...
    def delFun(self):
        ...
        try:
            ...
            self.table.delete(*self.table.get_children())
            tk.messagebox.showinfo("Success",f"Book.{name} of {opt} edition removed from store!")
